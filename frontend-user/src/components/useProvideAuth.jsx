...
function useProvideAuth() {
    ...
    React.useEffect(() => {
      const firebaseAuthListener = firebase.auth().onAuthStateChanged((user) => { // detaching the listener
        setUser(user)  
        if (user) {
    ...
    const signInWithEmail = (email, password, callback, callBackError) => {
      return authenticator.signInWithEmailPassword(email, 
        password, 
    ...
    const signUpWithEmail = (email,password, callback, callBackError) => {
      return authenticator.signUpWithEmailPassword(email, 
        password, 
        (user)=>setUserFromProvider(user, callback), 
        (error)=>setOnErrorFromProvider(error, callBackError));
    }
    ...
    const setPersistenceHelper = (persistentType=false, callback, callbackError) => {
